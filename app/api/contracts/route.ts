...
import { queryDocs, insertDoc, serializeDoc } from "@/lib/firebase/db";
...
export async function GET() {
  ...
  try {
    const data = await queryDocs("contracts", [], [{ field: "created_at", direction: "desc" }]);
    return NextResponse.json({ contracts: data.map(serializeDoc) });
  } catch (error) {
...
export async function POST(req: Request) {
  ...
  try {
    const data = await insertDoc("contracts", payload);

    await logActivity({
      userId, action: "Created", entityType: "contract", entityId: data.id,
      details: { target_name: data.title as string, status: data.status as string },
    });

    return NextResponse.json({ contract: serializeDoc(data) }, { status: 201 });
  } catch (error) {
